...
def process_accident(redis_conn, user_req):
	...
	hist_accident_key = "hist_accident_%s_%d_%d" % (day, accident_route_num, accident_loc)
	hist_users_key = "hist_users_%s_%d_%d_%s" % (day, accident_route_num, accident_loc, tod)
	accident_route_id = "curr:incident_route%d" % int(accident_route_num)
	if user_req[1][2]:
		process_accident_occur(redis_conn, accident_route_num, accident_loc, \
								accident_route_id, hist_accident_key, tod)
	else:
		accident_duration = int(user_req[1][3])
		process_accident_clear(redis_conn, accident_route_num, accident_loc, \
					accident_route_id, hist_accident_key, tod, accident_duration)
		
def process_accident_occur(redis_conn, accident_route_num, accident_loc, hist_accident_key, hist_users_key, tod, accident_route_id):
	print "In process_accident_occur()"
	redis_conn.incr("curr:incidents_reported_cnt")
	redis_conn.set(hist_accident_key, ' '.join([tod, ""]))
	redis_conn.sadd(accident_route_id, ' '.join([accident_loc, tod]))
...
def process_accident_clear(redis_conn, accident_route_num, accident_loc, \
                    accident_route_id, hist_accident_key,  hist_users_key, tod, accident_duration):
	print "In process_accident_clear()"
	redis_conn.incr("curr:incidents_cleared_cnt")
	redis_conn.srem(accident_route_id, str(accident_loc))
